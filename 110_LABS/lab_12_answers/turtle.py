...
def spiral(turtle, length, level):
    ...
    if level == 0: #base case, no recursion in this case
        turtle.draw(length) # just draws a line of length "length" from current  
                            # turtle position with current turtle direction
    #CHECK 2
...
def main():
    ...
    turtle.moveTo(Point(150, 150))
    turtle.turn(-pi / 2)
    turtle.draw(100)
    turtle.moveTo(Point(200, 150))
    turtle.turn(-pi / 2)
    turtle.draw(100)

    #CHECK 2
    turtle.moveTo(Point(100, 260))
    spiral(turtle, 100, 100)

    #CHECK 3 (with level 0)
