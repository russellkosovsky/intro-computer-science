...
def isPalindrome(phrase):
    smushed = smush(phrase) #first smush out extraneous characters
    revsmushed = reverse(smushed) #then reverse the smushed string
    if smushed == revsmushed: #check if it reads the same forward and backward
...
def main():
    ...
    win = GraphWin("LAB05- GUI & String Manipulation", 600, 600)

    #Entry object for user input
    inputBox = Entry(Point(300,200),50) 
    inputBox.draw(win)

    #Create and draw the GUI buttons
    ...
    drawButton(win, Point(500,10), Point(590,100), "Exit")
    ...
    while not (500 <= pt.getX() <= 590 and 10 <= pt.getY() <= 100):
        ...
        if userPhrase == "":
            output = "You didn't enter any text!"
        else:
