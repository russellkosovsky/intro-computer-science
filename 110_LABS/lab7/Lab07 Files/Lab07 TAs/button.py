...
def main():
    ...
    drawButton(win, Point(40,90), Point(60,96),"Exit")

    #While the Epoint is not in the Exit button ...
    while not (pt.getX() >= 40 and pt.getX() <= 60 and pt.getY() <= 96 and pt.getY() >= 90):

    #optional
        closeMessage = Text(Point(50,20), "Click the exit button to end program.")
