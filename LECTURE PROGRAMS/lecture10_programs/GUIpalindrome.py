...
# a GUI-based palindrome-checker intermediary tasks like smushing and reversing the text are also made available as features of the program

from graphics import *

########################################################################### STRING FUNCTIONS ###############################################################################

#lowercases then filters out all non-letters in the string s returns the resulting string
def smush(s):
    ...
    for char in s:
        ...
        if char >= "a" and  char <= "z":
            ...
            #...do we add it to the result string
            smushed = smushed + char
...
def reverse(s):
    ...
    for char in s:
        ...
        #add each character to the front of the result string 
...
def isPalindrome(s):
    ...
    #first smush out extraneous characters
    smushed = smush(s)

    #then reverse the smushed string
    revsmushed = reverse(smushed)

    #check if it reads the same forward and backward
    if smushed == revsmushed:
        #if so, yes, it's a palindrome!
        return True 
    else:
...
# in the GraphWin object gwin, this function draws a blue rectangular button with corners at Point objects pt1 and pt2
