...
def try_file():
    ...
    choice_msg = Text(Point(300, 275),"Do you want your file encoded decoded? type: Encoded or Decoded:")
    ...
    if choice1 == 'Encoded':
        ...
        for ch in MSG:
            ...
            if ch in list('abcdefghijklmnopqrstuvwxyz'):
                ...
                EncodedMSG2 = chr(ord('a') + (ord(ch) + key - ord('a')) % 26)
                EncodedMSG1 = EncodedMSG2 + EncodedMSG1

            elif ch in list('ABCDEFGHIJKLMNOPQRSTUVWXYZ'):

                EncodedMSG2 = chr(ord('A') + (ord(ch) + key - ord('A')) % 26)
                EncodedMSG1 = EncodedMSG2 + EncodedMSG1
            else:
                EncodedMSG1 = EncodedMSG1+ch
        fW.write(EncodedMSG1)
        #at the end of each ch the cipher is stored as EncodedMSG1 and is the written into the text file as fw.write(EncodedMSG1)
            
    elif choice1 == 'Decoded':
        fW = open(fname, "w")
        #the only change was for each option to include a writting segment stored as fw
        for ch in MSG:
            ...
            if ch in list('abcdefghijklmnopqrstuvwxyz'):
                ...
                DecodedMSG2 = chr(ord('a') + (ord(ch) - key - ord('a')) % 26)
                DecodedMSG1 =  DecodedMSG2 + DecodedMSG1
                ...
                DecodedMSG1 = DecodedMSG1+('')
            else:
                DecodedMSG1 = DecodedMSG1+ch
        fW.write(DecodedMSG1)
        #at the end of each ch the cipher is stored as DecodedMSG1 and is the written into the text file as fw.write(DecodedMSG1)
...
def encoder():
    ...
    Encodedresult = Text(Point(300, 340),"Your message has been successfully:")
...
#def usefile():
    ...
    #x and y cordiniates are found using the get() function
    #pt1 is set to x and pt2 is set to y
    click = myWin.getMouse()
##    pt1 = click.getX()
##    pt2 = click.getY()
##    if pt2>385 and pt2<410:
##        myWin.close()
